-- Enums are enforced as text + CHECK constraints on each column instead of
-- native Postgres enum types, so the allowed set can be changed with a simple
-- (transaction-safe) ALTER TABLE ... DROP/ADD CONSTRAINT. The values are
-- mirrored as Go constants in the application layer.
--
-- Original native-enum definitions kept here for reference:
--
